defmodule IslandsEngine.Rules do
  ...
  def check(%Rules{state: :players_set} = rules, {:position_islands, player}) when player in [:player1, :player2] do
  ...
  def check(%Rules{state: :players_set} = rules, {:set_islands, player}) when player in [:player1, :player2] do
  ...
  def check(%Rules{state: turn} = rules, {:win_check, win_or_not}) when turn in [:player1_turn, :player2_turn] do
    case win_or_not do
      :no_win -> {:ok, rules}
